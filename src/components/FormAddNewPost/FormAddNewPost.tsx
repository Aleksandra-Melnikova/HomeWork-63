...
const FormAddNewPost: React.FC<IFormAddNewPost> = ({
  title,
  formToOnePost,
  submitForm,
}) => {
  ...
  return (
    <>
      {title === "Add" || title === "Edit" ? (
        <h2 className="text-center mt-3">{title + " post"}</h2>
      ) : null}
      <div className="form-add-new-post p-5 border border-black-200 rounded-3 fs-5 mt-5">
        <form onSubmit={onSubmit}>
          ...
          <div className="mb-3 col">
            <label htmlFor="postMessage" className="form-label d-block">
              Text of post:
            </label>
            <textarea
              className="text-area mt-2 border border-black-200 rounded-3"
              id="postMessage"
              name="postMessage"
